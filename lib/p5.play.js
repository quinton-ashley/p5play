...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	class Sprite {
		constructor(x, y, w, h, collider) {
			...
			if (ani) {
				...
				if (this.tileSize) ts *= this.tileSize / ts;

				if (ani instanceof p5.Image) {
					w ??= ani.width / ts;
					if (this.shape != 'circle') h ??= ani.height / ts;
					this.addAni(ani);
				} else {
					if (typeof ani == 'string') this.changeAni(ani);
					else this._animation = ani;
					w ??= this.animation.width / ts;
					if (this.shape != 'circle') h ??= this.animation.height / ts;
				}
				if (w == 1) w = null;
				if (h == 1) h = null;
			}
		...
		constructor() {
			...
			if (typeof args[args.length - 1] != 'string' && !(args[args.length - 1] instanceof p5.Image)) {
				...
				if (!(sheet instanceof p5.Image) || !sheet.modified) {
					throw new Error(
						'The SpriteAnimation constructor can only accept p5.Image objects that have finished loading. You should use the async loadAni function!'
					);
				}
				...
				function _generateSheetFrames() {
					...
					// in the sheet
					let ts = world.tileSize;
					if (tileSize) ts *= tileSize / ts;
					x *= ts;
					y *= ts;

					if (!w || !h) {
						if (world.tileSize || tileSize) {
							w = h = 1 * ts;
	 ...
	 * @method
	 */
	Sprite.prototype.loadAni =
		Group.prototype.loadAni =
		Sprite.prototype.loadAnimation =
		Group.prototype.loadAnimation =
			async function () {
				return await p5.prototype.loadAni(this, ...arguments);
			};
	 ...
	 */
	Sprite.prototype.loadAnis =
		Group.prototype.loadAnis =
		Sprite.prototype.loadAnimations =
		Group.prototype.loadAnimations =
			async function (spriteSheet, atlases) {
	 ...
	 */
	p5.prototype.loadAni = p5.prototype.loadAnimation = async function () {
		let args = [...arguments];
		let parent, name;
		...
		for (let i = 0; i < args.length; i++) {
			if (typeof args[i] == 'string') {
				args[i] = await pInst.loadImage(args[i]);
			}
		}
	...
	p5.prototype.loadAnimation = p5.prototype.loadAni;
	...
	this.loadImage = function () {
		...
		return new Promise((resolve) => {
			_loadImage.call(pInst, url, (_img) => {
				_img.url = url;
				pInst.p5play.images[url] = _img;
				pInst.p5play.images.onLoad(_img);
				resolve(_img);
			});
		});
	};
	...
	// if (context.preload)
	// 	throw new Error("The old p5.js preload function can't be used with p5.play. Use an async setup function instead.");

	this._preloadCount = 10000000;

	// hijack the p5.js preload function!
	context.preload = async () => {
		context = this._isGlobal ? window : this;
		if (typeof context.setup == 'function') {
			let setup = context.setup;
			context.setup = null;
			await setup(); // async setup :)
			context.setup = null;
		} else return;

		// prevent createCanvas from running inside _setup
		this.createCanvas = () => {};
