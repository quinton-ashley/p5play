...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	class Sprite {
		constructor(x, y, w, h, collider) {
			...
			if (ani) {
				...
				if (ani.w != 1) w ??= ani.w / ts;
				if (this.shape != 'circle' && ani.h > 1) h ??= this.ani.h / ts;
			}
	...
	class SpriteAnimation {
		constructor() {
			...
			// SpriteSheet mode
				...
				if (sheet instanceof p5.Image && sheet.width > 1 && sheet.height) {
	...
	this.loadImage = function () {
		...
		if (typeof args[args.length - 1] == 'function') {
			img.cbs.push(args[args.length - 1]);
		}
		img.url = url;
