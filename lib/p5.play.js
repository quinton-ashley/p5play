...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	class Sprite {
		...
		rotate(angle, speed) {
			...
			let ang = this.angle + radians(angle);
	...
	class World extends pl.World {
		constructor(gravityX, gravityY, tileSize) {
			...
			tileSize ??= 0;
			this.origin = new pl.Vec2(width * 0.5, height * 0.5);
	...
	p5.prototype.drawSprites = function (group) {
		...
		for (let i = 0; i < group.length; i++) {
			...
			let x = width * 0.5 - world.origin.x + pos.x * world.tileSize;
			let y = height * 0.5 - world.origin.y + pos.y * world.tileSize;
			...
			this.push();
			x = fixRound(x) - (sprite.w % 2) * 0.5;
			y = fixRound(y) - (sprite.h % 2) * 0.5;
			this.translate(x, y);
