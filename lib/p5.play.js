	...
	class World extends pl.World {
		...
		set gravity(val) {
			if (typeof val == 'number') {
				this.m_gravity.y = pInst.round(val || 0);
			} else if (Array.isArray(val)) {
				this.setGravity(new pl.Vec2(pInst.round(val[0] || 0), pInst.round(val[1] || 0)));
			} else {
				this.setGravity(new pl.Vec2(pInst.round(val.x || 0), pInst.round(val.y || 0)));
			}
		}
