...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	class Sprite {
		constructor(x, y, w, h, collider) {
			...
			this.vel.x = 0;
			this.vel.y = 0;
			this.scale = group.scale || 1;
			this._mirrorX = group.mirrorX ? -1 : 1;
			this._mirrorY = group.mirrorY ? -1 : 1;
			this.drag = group.drag || 0;
			this.debug = group.debug || false;
			this._shift = {};
			this.idNum = spritesCreated;
			spritesCreated++;

			for (let g of this.groups) {
				let traits = Object.assign({}, g);

				// don't add these traits
				let deletes = [
					'idNum',
					'p',
					'parent',
	...
	class Group extends Array {
		constructor(...args) {
			...
			// 			if (val === undefined && _this.parent) {
			// 				val = _this.parent[prop];
			// 			}
			// 			if (typeof val == 'function') val = val();
			// 			return val;
			// 		},
		...
		addAni(name, atlas) {
			...
			let { size, pos, line, frames, frameDelay, rotation } = atlas;
			// size ??= this.tileSize || world.tileSize;
			size = 1;
			pos ??= line || 0;
			let sheet = this.spriteSheet || world.spriteSheet;
			let ani = createAni(sheet, size, pos, frames, frameDelay);
			if (rotation) ani.rotation = rotation;
			this.addAnimation(name, ani);
		}
	...
	p5.prototype.colorPal = (c, palette) => {
		if (typeof c == 'number' || (typeof c == 'string' && c.length > 1)) {
			return p5.prototype.color.call(this, c);
		}
		...
		if (!c) return p5.prototype.color.call(this, 0, 0, 0, 0); // transparent
		return p5.prototype.color.call(this, c);
	};
