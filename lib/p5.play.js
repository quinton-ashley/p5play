...
function PixelCollider(pInst, _center, _precision, _minAlpha, spriteInstance) {
  ...
  // TODO: Make these as inputs to the function:
  this.minAlpha = _minAlpha | 128;
  this.precision = _precision | 10;
  ...
  this.pixelWithPixel = function(pix, displacement) {
    ...
    if(displacement && this.sprite.debug) {
      stroke(178);
      strokeWeight(7);
      point(topLeft.x + middle.x, topLeft.y + middle.y);
    }
    ...
    if(collisionPoints.length > 0) {
      ...
      if(this.sprite.debug) {
        stroke(255,0,0);
        strokeWeight(5);
        point(topLeft.x + p.x, topLeft.y + p.y);
      }
      ...
      if(this.sprite.debug) {
        stroke(120, 0, 10);
        strokeWeight(1);
        line(topLeft.x + middle.x, topLeft.y + middle.y, topLeft.x + p.x, topLeft.y + p.y);
      }
  ...
  this.parseImage = function(img) {
    if(this.sprite.width !== 1 || this.sprite.height !== 1) {
      ...
      img._uuid = Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 5);
      this.data[img._uuid] = {
        img: img,
        filled: filled,
