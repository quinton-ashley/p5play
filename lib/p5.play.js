...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	pl.Fixture.prototype.shouldCollide = function (that) {
		...
		cb = _findContactCB('overlaps', b, a);
		if (cb) {
			if (typeof cb == 'function') {
				contacts.push([cb, b, a]);
			}
			return false;
		}
	 ...
	 * @param {String} physics is 'dynamic' by default, can be 'static', 'kinematic',
	 * or 'none'
	 */
	class Sprite {
		constructor(x, y, w, h, collider) {
			...
			if (x !== undefined && x instanceof Group) {
				...
				w = h;
				h = collider;
				collider = arguments[arguments.length - 1];
			}
			if (x !== undefined && typeof x != 'number') {
				...
				if (ani instanceof p5.Image) {
					...
					w ??= this.animation.images[0].frame.width;
					h ??= this.animation.images[0].frame.height;
				}
			}
		...
		/**
		 * In p5.play version 3 there is no need to use this method.
		 *
		 * @deprecated
		 * @private
		 */
		 ...
		 * sprite.position can not be set directly
		 *
		 * @property pos
		 */
		set pos(val) {
		...
		addAnimation(label) {
			...
			if (typeof label !== 'string') {
				...
				this.p.print('sprite.addAnimation error: you must specify a label and n frame images');
				return -1;
			} else if (arguments[1] instanceof SpriteAnimation) {
				let sourceAnimation = arguments[1];
				anim = sourceAnimation.clone();
			} else {
				let animFrames = [];
		...
		move(destX, destY, speed, cb) {
			...
			let perc = Math.max(1, speed / dist);

			this.vel.x = destX - this.x * perc * world.tileSize;
			this.vel.y = destY - this.y * perc * world.tileSize;
	...
	class Group extends Array {
		...
		addAnimation(label) {
			...
			if (typeof label !== 'string') {
				this.p.print('group.addAnimation error: the first argument must be a label (String)');
				return -1;
			} else if (arguments.length < 2) {
				this.p.print('group.addAnimation error: you must specify a label and n frame images');
				return -1;
			} else if (arguments[1] instanceof SpriteAnimation) {
				let sourceAnimation = arguments[1];
				anim = sourceAnimation.clone();
	...
	class World extends pl.World {
		constructor(gravityX, gravityY, tileSize) {
			...
			this.on('post-solve', this._contactListener);
			world = this;
		}
		...
		_contactListener(contact) {
			...
			// log(a, b);
			let cb = _findContactCB('collides', a, b);
			if (cb) {
				if (typeof cb == 'function') {
					contacts.push([cb, a, b]);
				}
			...
			cb = _findContactCB('collides', b, a);
			if (typeof cb == 'function') {
				contacts.push([cb, b, a]);
			}
		}
		...
		createTiles(tiles, x, y, w, h) {
			...
			for (let row = 0; row < tiles.length; row++) {
				for (let col = 0; col < tiles[row].length; col++) {
					...
					g.createSprite(anim, x + row, y + col, w, h);
				}
			}
		}
	}
