...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	class Sprite {
		constructor(x, y, w, h, collider) {
			...
			if (ani) {
				if (ani instanceof p5.Image) {
					...
					w ??= this.animation.width / world.tileSize;
					if (this.shape != 'circle') h ??= this.animation.height / world.tileSize;
				}
			}
		...
		addAni(name, atlas) {
			...
			let { size, pos, line, frames, delay, rotation } = atlas;
			size ??= [this.w * this.scale, this.h * this.scale];
			pos ??= line || 0;
			let spriteSheetImg = this.spriteSheet || world.spriteSheet;
			let ani = createAni(spriteSheetImg, size, pos, frames, delay);
			if (rotation) ani.rotation = rotation;
			this.addAnimation(name, ani);
		}
	...
	class SpriteAnimation {
		...
		draw(x, y, r) {
			...
			// if animation is used independently draw and update are the same
			...
			if (this.images[this.frame] !== undefined) {
				if (this.spriteSheet) {
					...
					let w = frame_info.width * world.tileSize;
					let h = frame_info.height * world.tileSize;
					...
					this.p.image(
						...
						w,
						h,
						frame_info.x * world.tileSize,
						frame_info.y * world.tileSize,
						w,
						h
					);
				} else {
	...
	class SpriteSheet {
		...
		drawFrame(frame_name, x, y, width, height) {
			...
			this.p.image(
				...
				x,
				y,
				dWidth * world.tileSize,
				dHeight * world.tileSize,
				frameToDraw.x,
				frameToDraw.y,
				frameToDraw.width * world.tileSize,
				frameToDraw.height * world.tileSize
			);
		}
	...
	class Group extends Array {
		...
		addAnimation() {
			...
			anim.isSpriteAnimation = true;
			this.animations[name] = anim;
			if (!this.animation) this._animation = anim;
			return anim;
		}
	...
	p5.prototype.createAni = function (spriteSheetImg, size, pos, frameCount, frameDelay) {
		...
		if (frameDelay !== undefined) ani.frameDelay = frameDelay;
		return ani;
	};
	...
	this.background = function () {
		let args = arguments;
		let c = p5.prototype.colorPal(args[0]);
