...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	this.cameraPush = function () {
		...
		camera.mouseX = this.mouseX + camera.x - this.width / 2;
		camera.mouseY = this.mouseY + camera.y - this.height / 2;

		if (!camera.active) {
			camera.active = true;
			this.push();
			this.scale(camera.zoom);
			this.translate(-camera.x + this.width / 2 / camera.zoom, -camera.y + this.height / 2 / camera.zoom);
		}
	};
