...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	let plScale = 60;
	this._p5play = {};
	this._p5play.wasUpdateSpritesCalledByUser = false;
	...
	class Sprite {
		constructor(x, y, w, h, bodyType) {
			...
			let dimensions;
			if (shape == 'box' || shape == 'circle') {
				dimensions = scaleTo({ x: w - 0.05, y: h - 0.05 });
			}
		...
		display() {
			...
			else if (this.animation) this.animation.draw(0, 0, 0);
			else {
				for (let fxt = this.fixtureList; fxt; fxt = fxt.getNext()) {
			...
			if (typeof angle === 'undefined') {
				if (this.velocity.x !== 0 || this.velocity.y !== 0) {
					...
					if (this.p._angleMode === this.p.RADIANS) {
						a = p5.prototype.radians(this._rotation);
					} else {
						a = this._rotation;
					}
				}
		...
		moveTowards(destX, destY, tracking) {
			tracking ??= 0.1;
			this.velocity.x = (destX - this.x) * tracking;
			this.velocity.y = (destY - this.y) * tracking;
		}
		...
		rotateTowards(angle, tracking) {
			// tracking ??= 0.1;
			// let ang = (radians(angle) - this.angle) * tracking;
			// console.log(radians(angle), this.angle, ang);
		...
		rotate(angle, speed) {
			...
			let ang = this.angle + p5.prototype.radians(angle);
		...
		displace(target, callback) {
			if (target instanceof Group) {
				for (let s of target) {
					this._displace(s, callback);
				}
				return;
			}
			this._displace(target, callback);
		}

		_displace(o, cb) {
			this.linearDamping = 0;
			o.linearDamping = o.mass;
		}
		...
		bounce(target, callback) {
			if (target instanceof Group) {
				for (let s of target) {
					this._bounce(s, callback);
				}
				return;
			}
			this._bounce(target, callback);
		}
	...
	class Group extends Array {
		...
		// 		if ((this.indexOf(a) && b == target) || target.indexOf(b)) {
		// 			log('hi');
		// 		}
		// 	});
		// }
	...
	class World extends pl.World {
		constructor(gravityX, gravityY, tileSize) {
			...
			tileSize ??= 0;
			this.origin = new pl.Vec2(this.p.width * 0.5, this.p.height * 0.5);
			this.tileSize = tileSize || 1;
			this.groups = [this.p.allSprites];
			// this.on('begin-contact', this.contactListener);
			world = this;
		}
		...
		// 	const b = contact.m_fixtureB.m_body.sprite;
		// 	// Get both bodies
		// 	// if (!a.body.isActive()) return;
		// 	// if (!b.body.isActive()) return;
		// 	// if (a.body.collision == null && b.body.collision == null) return;
		// 	// contacts.push(a, b);
		// 	log(a, b);
		// 	if (a._bounce) a._bounce(b);
		// 	if (b._bounce) b._bounce(a);
		// }
	...
	p5.prototype.updateSprites = function (timeStep, velocityIterations, positionIterations) {
		...
		this._p5play.wasUpdateSpritesCalledByUser = true;
	};
...
p5.prototype.registerMethod('pre', function p5playPreDraw() {
	if (this._p5play.wasUpdateSpritesCalledByUser) return;
	this.updateSprites();
	this._p5play.wasUpdateSpritesCalledByUser = false;
});
