...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	function _findOverlapCB(s0, s1) {
		let cb = s0.overlaps[s1];
		if (cb) return cb;

		for (let group of s1.groups) {
			cb = s0.overlaps[group];
			if (cb) return cb;
			for (let g2 of s0.groups) {
				cb = group.overlaps[g2];
				if (cb) return cb;
			}
		}
		return;
	...
	pl.Fixture.prototype.shouldCollide = function (that) {
		...
		const a = this.m_body.sprite;
		const b = that.m_body.sprite;

		let cb = _findOverlapCB(a, b);
		if (cb) {
			if (typeof cb == 'function') cb(a, b);
			return false;
		}
		cb = _findOverlapCB(b, a);
		if (cb) {
			if (typeof cb == 'function') cb(b, a);
			return false;
		}
	...
	class Sprite {
		constructor(x, y, w, h, bodyType) {
			...
			if (x !== undefined && x instanceof Group) {
				let group = x;
			...
			props.shape = s;
			props.density ??= 5;
			props.friction ??= shape != 'circle' ? 0.5 : 0;
			props.restitution ??= shape != 'circle' ? 0.2 : 1;
			...
			this.position = {
				get x() {
					let x = (_this.body.getPosition().x / world.tileSize) * plScale;
					return fixRound(x);
				},
				set x(val) {
					this.destX = val;
					let pos = new pl.Vec2((val * world.tileSize) / plScale, _this.body.getPosition().y);
					_this.body.setPosition(pos);
				},
				get y() {
					let y = (_this.body.getPosition().y / world.tileSize) * plScale;
					return fixRound(y);
				},
				set y(val) {
					this.destY = val;
					let pos = new pl.Vec2(_this.body.getPosition().x, (val * world.tileSize) / plScale);
					_this.body.setPosition(pos);
				}
			};
		...
		/**
		 * The sprite's horizontal mirroring.
		 *
		 * @property mirrorX
		 * @param {Boolean} true if the sprite should be mirrored
		 */
		set mirrorX(val) {
		...
		/**
		 * The sprite's vertical mirroring.
		 *
		 * @property mirrorY
		 * @param {Boolean} true if the sprite should be mirrored
		 */
		...
		// }
		get speed() {
			return this.p.createVector(this.velocity.x, this.velocity.y).mag();
		}
		/**
		 * The sprite's speed.
		 *
		 ...
		 * custom drawing function. The 0,0 point will be the center of the
		 * sprite. Example: sprite.draw = function() { ellipse(0,0,20,10) }
		 * Will display the sprite as an oval.
		 *
		 * @method draw
		 */
		draw() {
		...
		setSpeed(speed, angle) {
			...
			this.velocity.x = this.p.cos(angle) * speed;
			this.velocity.y = this.p.sin(angle) * speed;
		}
		...
		getDirection() {
			if (this.velocity.x !== 0 || this.velocity.y !== 0) {
				return this.p.atan2(this.velocity.y, this.velocity.x);
			} else {
				return this.angle;
			}
		}
		...
		moveTowards(destX, destY, tracking) {
			tracking ??= 0.1;
			this.velocity.x = (destX - this.x) * tracking * world.tileSize;
			this.velocity.y = (destY - this.y) * tracking * world.tileSize;
		}
		...
		move(destX, destY, speed, cb) {
			...
			return (async () => {
				...
				do {
					...
					distX = Math.abs(this.x - this.destX);
					distY = Math.abs(this.y - this.destY);
		...
		attractionPoint(magnitude, x, y) {
			let angle = this.p.atan2(y - this.position.y, x - this.position.x);
		...
		rotateTowards(angle, tracking) {
			tracking ??= 0.1;
			console.log(angle, this.angle, ang);
			this.angularVelocity = (angle - this.angle) * tracking;
		}
		...
		rotate(angle, speed) {
			...
			let ang = this.angle + angle;
			let mod = ang - this.angle > 0 ? 1 : -1;
			this.angularVelocity = speed * mod;

			return (async () => {
				let cw = ang > this.angle;
				while ((cw && this.angle < ang) || (!cw && this.angle > ang)) {
					await p5.prototype.delay();
				}
		 ...
		 *
		 * @method ani
		 * @param {...String} anis the names of one or many animations to be played in sequence
		 */
		async ani(...anis) {
		...
		addAni(name, atlas) {
			...
			pos ??= line || 0;
			this.addAnimation(name, createAni(this.spriteSheet, size, pos, frames, frameDelay));
		}
	...
	class Group extends Array {
		constructor(...args) {
			...
			this.groupID = 'g' + groupCount++;

			if (world) world.groups.push(this);
		}

		sprite() {
			return new Sprite(this, ...arguments);
		}
		 ...
		 * @param {Function} [cb] Given the sprite that passed the cull boundary, if no callback is given the sprite is removed by default
		 */
	...
	class World extends pl.World {
		constructor(gravityX, gravityY, tileSize) {
			...
			this.on('pre-solve', this._contactListener);
			world = this;
		}
		 ...
		 */
		_findContactCB(s0, s1) {
			let cb = s0.collides[s1];
			if (cb) return cb;
			cb = s0.overlaps[s1];
			if (cb) return cb;

			for (let group of s1.groups) {
				cb = s0.collides[group];
				if (cb) return cb;
				cb = s0.overlaps[group];
				if (cb) return cb;
				for (let g2 of s0.groups) {
					cb = group.collides[g2];
					if (cb) return cb;
					cb = group.overlaps[g2];
					if (cb) return cb;
				}
		 ...
		 * @private
		 */
		_contactListener(contact) {
			// Get both fixtures
			const a = contact.m_fixtureA.m_body.sprite;
			const b = contact.m_fixtureB.m_body.sprite;
			// log(a, b);
			let cb = this._findContactCB(a, b);
			if (cb) {
				if (typeof cb == 'function') cb(a, b);
				return;
			}
			cb = this._findContactCB(b, a);
			if (cb) {
				if (typeof cb == 'function') cb(b, a);
			}
		}
		...
		get offsetX() {
			return -this.origin.x - this.p.width * 0.5;
		}
		/**
		 * @property offsetX
		 */
		set offsetX(val) {
			this.origin.x = this.p.width * 0.5 - val;
		}
		get offsetY() {
			return -this.origin.y - this.p.height * 0.5;
		}
		/**
		 * @property offsetY
		 */
		set offsetY(val) {
			this.origin.y = this.p.height * 0.5 - val;
		}
	}
	...
	// p5.prototype.resizeCanvas = (w, h, noRedraw) => {
	// 	_resizeCanvas.call(this, ...arguments);
	// 	this.centerX = w * 0.5;
	// 	this.centerY = h * 0.5;
	// };
});
