...
p5.prototype.registerMethod('init', function p5PlayInit() {
	 ...
	 *
	 * Look at the examples to learn how to create many different
	 * kinds of sprites.
	 *
	 * @example
	 *
	 ...
	 *
	 *   let circle = new Sprite(x, y, radius);
	 *
	...
	class Sprite {
		 ...
		 *
		 * It is more efficient to set the physics body type of the sprite to 'none'
		 *
		...
		set rotationSpeed(val) {
			...
			else this._angularVelocity = val;
		}
		// get physicsBody() {
		...
		update() {
			...
			// }
			if (this._angularVelocity) this.angle += this._angularVelocity;
		}
		...
		/**
		 * Deprecated: set direction and speed seperately
		 *
		...
		overlap(target, callback) {
			...
			return world.touching[this + target];
		}
	...
	class SpriteAnimation {
		constructor() {
			...
			if (
				...
				// they are just two seperate images with numbers
		...
		clone() {
			...
			myClone.offsetX = this.offsetX;
			myClone.offsetY = this.offsetY;
		...
		draw(x, y, r) {
			...
			if (this.images[this.frame] !== undefined) {
				if (this.spriteSheet) {
					...
					this.p.image(this.images[this.frame], this.offsetX, this.offsetY);
				}
			} else {
		...
		/**
		 * The method was deprecated because it has a confusing name.
		 * It seems like it should play the animation in reverse but it
		 * just changes frame to the first frame.
		 *
		 * @private
		 * @deprecated
		 * @method rewind
	...
	this.createCanvas = function () {
		_createCanvas.call(pInst, ...arguments);
	...
	this.resizeCanvas = (w, h, noRedraw) => {
