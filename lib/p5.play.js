...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	class Sprite {
		constructor(x, y, w, h, collider) {
			...
			this.mirror = {
				get x() {
					return _this._mirror.x;
				},
				set x(val) {
					_this._mirror.x = val ? -1 : 1;
				},
				get y() {
					return _this._mirror.y;
				},
				set y(val) {
		 ...
		 */
		get mirrorX() {
			return this._mirror.x;
		}
		set mirrorX(val) {
			this._mirror.x = val ? -1 : 1;
		}
		/**
		 ...
		 */
		get mirrorY() {
			return this._mirror.y;
		}
		set mirrorY(val) {
			this._mirror.y = val ? -1 : 1;
		}
		...
		set d(val) {
			...
			if (prevShape != 'circle' || prevShape != 'box') {
		...
		_display() {
			...
			this.p.scale(this.scale * this.mirror.x, this.scale * this.mirror.y);
	 ...
	 *
	 * Since groups just contain references, a sprite can be in multiple
	 * groups and deleting a group doesn't delete the sprites themselves.
	 *
	...
	class Group extends Array {
		constructor(...args) {
			...
			if (this.p.world) this.p.world.groups[this.idNum] = this;
	...
	p5.prototype.updateSprites = function (timeStep, velocityIterations, positionIterations) {
		...
		if (this.mouse.left == -1 && this.p5play.mouseSprite) {
			// mouse released on the sprite only if it's being
			// hovered over
			if (this.p5play.mouseSprite.mouse.hover > 0) {
				this.p5play.mouseSprite.mouse.left = -1;
			} else {
				this.p5play.mouseSprite.mouse.left = 0;
				this.p5play.mouseSprite.mouse.draggable = false;
			}
...
p5.prototype.registerMethod('pre', function () {
	...
	if (this.frameCount == 1) {
		...
		const _mousePressed = this.mousePressed;

		this.mousePressed = function () {
			if (this.mouseButton == this.LEFT) this.mouse.left = 1;
			else if (this.mouseButton == this.CENTER) this.mouse.center = 1;
			else if (this.mouseButton == this.RIGHT) this.mouse.right = 1;
			if (_mousePressed) _mousePressed.call(this, ...arguments);
		};
	}
});

p5.prototype.registerMethod('post', function p5playPostDraw() {
	if (this.mouse.left == -1) this.mouse.left = 0;
	if (this.mouse.center == -1) this.mouse.center = 0;
	if (this.mouse.right == -1) this.mouse.right = 0;

	if (this.mouseIsPressed) {
		if (this.mouseButton == this.LEFT) this.mouse.left++;
		else if (this.mouseButton == this.CENTER) this.mouse.center++;
		else if (this.mouseButton == this.RIGHT) this.mouse.right++;
	} else {
		this.mouse.left = this.mouse.left > 0 ? -1 : 0;
		this.mouse.center = this.mouse.center > 0 ? -1 : 0;
		this.mouse.right = this.mouse.right > 0 ? -1 : 0;
	}
