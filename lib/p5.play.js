...
p5.prototype.registerMethod('init', function p5PlayInit() {
	...
	class Sprite {
		constructor(x, y, w, h, collider) {
			 ...
			 */
			this.shapeColor = group.shapeColor || this.p.color(this.p.random(255), this.p.random(255), this.p.random(255));
			...
			this.vel = {
				get x() {
					...
					return fixRound(val / world.tileSize);
				},
				set x(val) {
					val *= world.tileSize;
					if (_this.body) {
			...
			if (collider != 'none' && collider != 'n') {
				...
				this.x = x;
				this.y = y;
				this.w = w || (world.tileSize > 1 ? 1 : 100);
				this.h = h || this.w;
			}
		...
		addCollider(offsetX, offsetY, w, h) {
			...
			if (shape == 'box') {
				...
				if (!usesVertices) {
					...
					for (let i = 0; i < vecs.length; i++) {
						let vec = vecs[i];
						vecs[i] = new pl.Vec2((vec.x * world.tileSize) / plScale, (vec.y * world.tileSize) / plScale);
					}
				}
				if (shape == 'polygon') {
		...
		_resize() {
			for (let fxt = this.fixtureList; fxt; fxt = fxt.getNext()) {
				let sh = fxt.m_shape;
				let v = scaleTo({ x: this.w, y: this.h });
				if (sh.m_type == 'circle') {
					sh.m_radius = v.x * 0.5;
				}
			}
		...
		move(destX, destY, speed, cb) {
			...
			this.vel.x = (destX - this.x) * percent;
			this.vel.y = (destY - this.y) * percent;
			...
			return (async () => {
				...
				} while (distX > margin || distY > margin);
	...
	class SpriteAnimation {
		constructor() {
			...
			// SpriteSheet mode
			if (typeof args[args.length - 1] != 'string' && !(args[args.length - 1] instanceof p5.Image)) {
				let sheet, atlas;
				...
				if (parent) sheet ??= parent.spriteSheet;
				if (world) sheet ??= world.spriteSheet;

				let _this = this;

				if (!(sheet instanceof p5.Image) || !sheet.modified) {
					throw new Error(
						'The SpriteAnimation constructor can only accept images that were loaded using the async loadImage function.'
					);
				}
				...
				function _generateSheetFrames() {
					...
					if (world) tileSize ??= world.tileSize;
					...
					if (tileSize) {
						let ts = tileSize / world.tileSize;
						w *= ts;
						h *= ts;
					}
		...
		draw(x, y, r) {
			...
			if (this.images[this.frame] !== undefined) {
				if (this.spriteSheet) {
					let frame_info = this.images[this.frame];
					let { x, y, w, h } = frame_info;
					if (world) {
						x *= world.tileSize;
						y *= world.tileSize;
						w *= world.tileSize;
	...
	class Group extends Array {
		constructor(...args) {
			...
			super(...args);
			this.p = pInst;
			this._isAllSpritesGroup = !world;

			if (this.p.allSprites && !world) world = new World();
	 ...
	 * @method
	 */
	Sprite.prototype.loadAni = Group.prototype.loadAni = async function () {
		return await p5.prototype.loadAnimation(this, ...arguments);
	};

	Sprite.prototype.loadAnimation = Group.prototype.loadAnimation = async function () {
		return await p5.prototype.loadAnimation(this, ...arguments);
	};
	 ...
	 */
	Sprite.prototype.addAni = Group.prototype.addAni = function () {
		let args = [...arguments];
		let name, anim;
		if (args[0] instanceof SpriteAnimation) {
			...
			anim = new SpriteAnimation(this, ...args);
			name = anim.name;
		}
	 ...
	 * @method
	 */
	Sprite.prototype.addAnimation = Group.prototype.addAnimation = function () {
		return this.addAni(...arguments);
	};

	/**
	 * Load multiple animations from a SpriteSheet
	 *
	 * TODO error checking
	 *
	 * @method
	 * @param {p5.Image} spriteSheet
	 * @param {Object} atlases
	 */
	Sprite.prototype.loadAnis = Group.prototype.loadAnis = async function (spriteSheet, atlases) {
		this.spriteSheet = await this.p.loadImage(spriteSheet);
		this.addAnis(atlases);
	};
	 ...
	 * @method
	 */
	Sprite.prototype.loadAnis = Group.prototype.loadAnis = async function () {
		return this.loadAnis(...arguments);
	};
	 ...
	 * @method addAnis
	 */
	Sprite.prototype.addAnis = Group.prototype.addAnis = function () {
		let args = arguments;
		let atlases;
		if (args.length == 1) {
			atlases = args[0];
			...
			throw new Error('Use the async loadAni function instead!');
		}
	 ...
	 * @method
	 */
	Sprite.prototype.addAnimations = Group.prototype.addAnimations = function () {
		this.addAnis(...arguments);
	};
	...
	class World extends pl.World {
		...
		resize() {
			this.origin = {
				x: this.p.width * 0.5 + this.offset.x,
				y: this.p.height * 0.5 + this.offset.y
			};
			if (this.tileSize != 1) {
				this.origin.x -= this.tileSize * 0.5;
				this.origin.y -= this.tileSize * 0.5;
			}
		}
		...
		createTiles(tiles, x, y, w, h) {
			...
			for (let row = 0; row < tiles.length; row++) {
				for (let col = 0; col < tiles[row].length; col++) {
					...
					new g.Sprite(anim, x + col, y + row, w, h);
				}
			}
		}
	}
	...
	p5.prototype.updateSprites = function (timeStep, velocityIterations, positionIterations) {
		...
		let sprites = p5.prototype.getSpritesAt(this.mouseX, this.mouseY);
	 ...
	 * @returns
	 */
	p5.prototype.getSpritesAt = function (x, y) {
	...
	p5.prototype.getSpriteAt = function (x, y) {
		let sprites = p5.prototype.getSpritesAt(x, y);
		sprites.sort((a, b) => (a.layer - b.layer) * -1);
		return sprites[0];
	};
	...
	p5.prototype.colorPal = (c, palette) => {
		...
		// if transparent
		if (clr === '') return p5.prototype.color.call(this, 0, 0, 0, 0);
		return p5.prototype.color.call(this, clr || c);
	};
	 ...
	 *
	 * In v3 this function is run automatically at the end of the p5.js
	 * draw loop, unless it was run within the draw loop.
	 *
	 * @deprecated
	 * @method drawSprites
	 ...
	 */
	p5.prototype.loadAnimation = async function () {
		let args = [...arguments];
		let parent, name;
		...
		if (args.length == 2 && typeof args[0] == 'string' && (typeof args[1] == 'string' || typeof args[1] == 'number')) {
			...
			if (to && prefix1 != prefix2) {
				this.images.push(await pInst.loadImage(from));
				this.images.push(await pInst.loadImage(to));
			} else {
				...
				if (!to || digits1 == digits2) {
					...
					for (let i = num1; i <= num2; i++) {
						...
						args.push(await pInst.loadImage(fileName));
					}
				} // case: case img1, img2
				else {
					for (let i = num1; i <= num2; i++) {
		...
		for (let i = 0; i < args.length; i++) {
			let arg = args[i];
			if (typeof arg == 'string' && arg.length != 1 && arg.includes('.')) {
				args[i] = await pInst.loadImage(arg);
			}
		}
	...
	this.loadImage = function () {
		...
		if (img) return img;
		return new Promise((resolve) => {
			_loadImage.call(pInst, url, (_img) => {
				_img.url = url;
				images[url] = _img;
				resolve(_img);
			});
	...
	// hijack p5.js preload!
	context.preload = async () => {
		context = this._isGlobal ? window : this;
		...
		// prevent createCanvas from running twice inside _setup
		this.createCanvas = () => {};

		this._setup(); // finish setup
		context._preloadCount = 0; // finished preloading
		this._runIfPreloadsAreDone(); // remove loading screen
		this._draw(); // start draw looping!
	};

	this._preloadCount = 10000000;
});
